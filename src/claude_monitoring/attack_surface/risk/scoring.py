...
**Ratifications applied (Rajan 2026-06-07):**

- **Q1:** unknown-capability floor at 40 → MEDIUM; signature uses set
  difference (NOT tag count) so credential-bearing unrecognized MCPs
  do not escape via ``secrets_access``.
- **Q2:** :class:`RiskBand` is a ``(str, enum.Enum)`` mixin.
- **Q3:** weights snapshotted onto each :class:`RiskScoreResult` at
  compute time from :data:`weights.FACTOR_WEIGHTS` (single source of
  truth + audit-stable persistence).
- **Q4:** orphan ``DATA_EXFILTRATION_CAPABLE`` tag raises via explicit
  ``if orphan: raise`` (NOT ``assert`` — strippable under ``python -O``;
  memory ``feedback_assert_strippable_use_if_raise.md``). Strict at
  this unit; the orchestrator wraps per-asset scoring with try/except
  so one orphan quarantines that asset and the scan continues.
...
def _check_orphan_derived_tag(ontology_tags: frozenset[OntologyCategory]) -> None:
    """Q4 ratification (Rajan 2026-06-07): if ``DATA_EXFILTRATION_CAPABLE``
