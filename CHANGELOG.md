...
- **HTTPS proxy now accepts both IPv4 and IPv6 connections (closes #70).** Pre-fix mitmproxy bound only to `0.0.0.0:9080` (IPv4). When macOS Happy Eyeballs raced IPv4 and IPv6 SYN packets and IPv6 won, Claude Desktop's connections arrived at mitmproxy and were refused — Vigil silently lost the traffic. Ground-truth verification on 2026-06-01 confirmed: Claude Desktop main process had 1 connection via proxy AND 2 simultaneous IPv6-direct connections to `[2607:6bc0::10]:443`. Architectural review confirmed the OS-level routing was correct (`networksetup -setsecurewebproxy` is stack-agnostic); the bypass was purely mitmproxy's listener config. Fix: add `--listen-host ::` to the mitmdump cmdline. On macOS, `IPV6_V6ONLY=0` is the BSD default, so a socket bound to `::` accepts both IPv4-mapped and native IPv6 connections — one dual-stack listener replaces the prior IPv4-only one. **Existing daemons must be restarted to pick up the new listener** (`ai-monitor --stop && ai-monitor --start --with-proxy`). On Linux the default is `IPV6_V6ONLY=1`; a Linux port would need either a sysctl change or two separate listeners. See `docs/design/ipv6-proxy.md`.
