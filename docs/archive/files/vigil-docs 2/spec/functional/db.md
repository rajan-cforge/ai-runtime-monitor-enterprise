...
```python
def insert_api_call(db, fields: dict) -> int | None:
    """Insert an api_calls row. Returns the rowid or None on failure.
    
    Called by watch.py::ClaudeWatchAddon for every captured response.
    """
```
...
- **Add a new insert helper:** follow the `insert_api_call` pattern (dict in, rowid out, returns None on failure)
