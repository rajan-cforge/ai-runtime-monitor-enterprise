...
- **Network Extension capture architecture (v0.3):** replace HTTPS-proxy + macOS system-proxy routing with Apple's Network Extension framework (NEPacketTunnelProvider / NEFilterDataProvider). NE intercepts at the OS network stack regardless of subprocess or address family, closing the v0.2.1 capture-coverage gaps for Electron desktop AI apps (Claude Desktop main-process IPv6 channel, ChatGPT Desktop with `chatgpt.com` outside `allow_hosts`, Cursor plugin-helper bypass). Distribution model shifts to signed `.pkg` + system-extension approval. Apple Developer Program enrollment and Network Extension entitlement approval are prerequisites; Apple's typical approval clock is 2-4 weeks. The entitlement application is structured around Vigil's documented use case — local security monitoring, open source, no remote data transmission — all of which fit Apple's published acceptable-use space for Network Extension entitlements. The v0.2.1 sprint considered three alternatives before recommending NE: PAC (proxy auto-config) was empirically tested and routes a subset of Electron traffic but cannot redirect a main-process persistent channel; dual-host `networksetup` was rejected at the macOS API level (single `HTTPSProxy` slot in the SC dictionary, verified empirically); and transparent proxying via `pf` packet-filter rules has its own constraints (TLS SNI sniffing, Electron HTTP/2 connection reuse complicates the redirect) and was not explored to the same depth.
