...
- **Network Extension capture architecture (v0.3):** replace HTTPS-proxy + macOS system-proxy routing with Apple's Network Extension framework (NEPacketTunnelProvider / NEFilterDataProvider). NE intercepts at the OS network stack regardless of subprocess or address family, with the goal of closing the v0.2.1 capture-coverage gaps for Electron desktop AI apps (Claude Desktop main-process IPv6 channel, ChatGPT Desktop with `chatgpt.com` outside `allow_hosts`, Cursor plugin-helper bypass). Distribution model would shift to signed `.pkg` + system-extension approval. Requires Apple Developer Program enrollment and a Network Extension entitlement approved by Apple — approval is not guaranteed and the typical clock is 2-4 weeks. The v0.2.1 PAC investigation empirically proved that PAC routes a subset of Electron traffic but cannot redirect a main-process persistent channel; NE is the mechanism we've identified for v0.3, gated on Apple approval and a POC that empirically confirms NE can see the bypassed traffic. Alternative paths (e.g., transparent proxying via `pf` rules) have their own constraints and have not been explored to the same depth.
