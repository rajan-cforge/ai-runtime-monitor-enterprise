...
> **v0.2.1 capture-coverage note (macOS Electron apps).** This boundary covers what the proxy CAN intercept when traffic is routed through it. On macOS the proxy is configured via `networksetup -setsecurewebproxy` at a single IPv4 host, and Electron-based AI apps (Claude Desktop, ChatGPT Desktop) split networking across child processes. The network-service helper honors that proxy for routine traffic, but the main process may maintain persistent IPv6 channels to AI APIs that bypass the IPv4 system proxy entirely. PAC (proxy auto-config) was empirically tested in the v0.2.1 cycle: it routes a subset of Electron traffic (validated for native CFNetwork apps; partial for Electron network helpers) but cannot redirect already-established main-process channels. This is **not** a boundary-violation issue — the proxy does what it's designed to do; the bypassed traffic simply never reaches B4. The architectural answer (v0.3) is Apple's Network Extension framework, which intercepts at the OS network stack regardless of which subprocess or address family. `ai-monitor --status` surfaces the live capture state per surface so users see the honest verdict rather than a uniform "captured" label. Trust-boundary properties (NameConstraints, no-modification, masking, auto-purge) are unaffected.
