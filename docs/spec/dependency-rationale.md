...
- **`mitmproxy>=10.0` (extra: `watch`)** — the `claude-watch` CLI and the `ClaudeWatchAddon` interception layer. mitmproxy is the canonical Python HTTPS-MITM toolkit; it ships an addon framework, certificate management, and a flow API that map directly onto Vigil's interception needs. The `[watch]` extra exists because the runtime daemon does NOT require mitmproxy — proxy capture is opt-in (`--with-proxy`), and most users will install the base package without it. Pinning at `>=10.0` because the addon API stabilized in v10. Alternatives considered: `proxy.py` (smaller surface, smaller community, no addon ecosystem), writing a custom MITM with `aiohttp` (re-implementing certificate trust dance and CONNECT tunneling is a security footgun).
