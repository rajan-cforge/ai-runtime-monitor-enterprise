...
commit body. The original commits on the feature branch are
discarded when the branch is deleted.
...
- Long-running incident response with documented incremental progress

Example: PR #20 (Phase 3A C1-C4) used rebase-merge because each of the
four security fixes was a self-contained change that should remain
...
Disabled at the repo level. Linear history required. The
"Create a merge commit" button is off; CI rejects merge bubbles.
