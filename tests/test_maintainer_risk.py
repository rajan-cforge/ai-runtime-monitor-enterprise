...
class TestEnrichedNpmMetadata:
    ...
    def test_npm_publisher_change_detected(self, mock_urlopen):
        ...
        mock_resp = MagicMock()
        mock_resp.read.return_value = json.dumps({
            "description": "x", "maintainers": [{"name": "attacker"}],
            "author": {"name": "original"}, "license": "MIT", "repository": {},
            "time": {"created": "2020-01-01T00:00:00Z", "1.0.0": "2020-01-01T00:00:00Z", "2.0.0": "2026-04-05T00:00:00Z"},
            "dist-tags": {"latest": "2.0.0"},
            "versions": {
...
class TestMaintainerRiskScoring:
    ...
    def test_no_source_repo(self):
        ...
        meta = {
            "first_published": "2020-01-01T00:00:00+00:00",
            "has_description": True, "has_repository": False, "has_source_repo": False,
        }
        score, reasons = assess_registry_risk("pkg", "npm", meta)
...
class TestMaintainerHistory:
    ...
    def test_no_change(self, db):
        ...
        result = detect_maintainer_changes("express", "npm",
            {"maintainers": [{"name": "dev1"}], "publisher": "dev1", "latest_version": "5.0.1"}, db)
        assert result["changed"] is False

    def test_publisher_change_detected(self, db):
        from claude_monitoring.threat_intel import detect_maintainer_changes

        detect_maintainer_changes("pkg", "npm",
            {"maintainers": [{"name": "dev1"}], "publisher": "dev1", "latest_version": "1.0.0"}, db)
        result = detect_maintainer_changes("pkg", "npm",
            {"maintainers": [{"name": "attacker"}], "publisher": "attacker", "latest_version": "1.0.1"}, db)
    ...
    def test_new_maintainer_detected(self, db):
        ...
        detect_maintainer_changes("pkg", "npm",
            {"maintainers": [{"name": "dev1"}], "publisher": "dev1", "latest_version": "1.0.0"}, db)
        result = detect_maintainer_changes("pkg", "npm",
            {"maintainers": [{"name": "dev1"}, {"name": "newguy"}], "publisher": "dev1", "latest_version": "1.0.1"}, db)
        assert result["changed"] is True
